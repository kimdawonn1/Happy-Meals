...
shinyServer(function(input, output, session) {
  ...
  output$headerText <- renderUI({
    p("In this page, feel free to read about each Global Factor we have selected. For each, we have provided a current definition and why we decided to include it.",
      style = "font-size:15px; line-height:1.8; color:#2c3e50;")
  })
  
  #Define adult obesity text
  output$obesityText <- renderUI({
    tagList(
      p("Adult obesity is a serious health condition that affects all parts of the body's systems. It is highly correlated to outcomes such as hypertension, heart disease, cancer, type II diabetes, sleep apnea, osteoarthritis, and more. It is also strongly connected to an individual or community's economic constraints, with some of the largest predictors of future obesity being access to healthy food, nutrition education, and sedentary lifestyles.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
      p("BMI (body mass index) is a commonly used measurement of body fat based on an individual's height and weight. Individuals with a BMI of 25 or higher are classified as overweight, while individuals with a BMI of 30 or higher are classified as clinically obese.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
      p("With this in mind, we have chosen the percentage of people in a given country with a BMI of 30 or higher to represent adult obesity.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
      p("However, BMI does have some flaws and limitations. For example, although a high BMI is generally related to increased body fat, this measure's use of body weight and height does not account for overall body composition or differences between races and sexes.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;")
    )
  })
  
  
  output$gdpText <- renderUI({
    tagList(
      p("Gross domestic product (GDP) is the total market value of all goods and services produced by a country over one year and serves as the most common indicator of economic performance. We chose it because prosperity and economic health are generally correlated to a higher standard of living through increased access to goods and services, as well as stronger healthcare and education. However, depending on how resources are distributed in a given society, this may not necessarily be the case.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
      p("In our app, you will find a country's gross GDP in a given year as well as a country's GDP per capita (per person).",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;")
    )
  })
  
  output$giniText <- renderUI({
    tagList(
      p("The Gini index (or Gini coefficient) is a measure of income inequality within a given population. It ranges from 0 to 1, with perfect equality represented by 0 (everyone has equal wealth) and perfect inequality represented by 1 (one person has all the wealth and nobody else has any).",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
      p("A higher Gini index indicates greater inequality, meaning wealth is concentrated in fewer hands. In contrast, lower values imply a more equitable distribution of wealth. We included this variable because 1) inequality affects how gross GDP translates to individual quality of life, and 2) we wanted to investigate how inequality correlates with factors like adult obesity and happiness.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;")
    )
  })
  
  output$happinessText <- renderUI({
    tagList(
      p("The World Happiness Report assesses and ranks countries according to factors that influence subjective well-being and happiness. Of these factors — including GDP per capita, social support, freedom to make life choices, and generosity — we selected positive affect.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
      p("Positive affect specifically refers to the frequency of positive emotions like joy and satisfaction. A higher score indicates that the population tends to feel happy and content more often. We chose this specific measure because we felt it most directly represented general happiness and subjective quality of life.",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;")
    )
  })
  ...
  output$year_info <- renderUI({
    tags$ul(style = "padding-left:16px; font-size:13px; color:#555; line-height:1.9;",
      tags$li("Adult Obesity: 1975 - 2016"),
      tags$li("Gross GDP: 1988 - 2016"),
      tags$li("Gini Inequality Index: 1975 - 2005"),
      tags$li("Happiness Index: 2005 - 2016")
    )
  })
  ...
  #CORRELATIONAL STUFF
  output$pearsoncorrelationText <- renderUI({
    p("Below you can explore the Pearson correlations between the global factors in our dataset. Select a pair of variables from the dropdown to see a scatter plot with a line of best fit and the correlation coefficient.",
      style = "font-size:15px; line-height:1.8; color:#2c3e50;")
  })
  
  # Define Pearson Correlation
  output$correlationdefinitionText <- renderUI({
    tagList(
      p("A Pearson correlation measures the strength and direction of the linear relationship between two variables. The result is a coefficient (r) ranging from -1 to 1:",
        style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
      tags$ul(
        tags$li(strong("r = 1"), " — perfect positive relationship (both variables increase together)", style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
        tags$li(strong("r = 0"), " — no linear relationship", style = "font-size:15px; line-height:1.8; color:#2c3e50;"),
        tags$li(strong("r = -1"), " — perfect negative relationship (one increases as the other decreases)", style = "font-size:15px; line-height:1.8; color:#2c3e50;")
      )
    )
  })
  ...
  output$rawdescText <- renderUI({
    p("Select a global factor from the dropdown below to explore its raw dataset. You can also choose which columns to display using the checkboxes.",
      style = "font-size:15px; line-height:1.8; color:#2c3e50;")
  })
